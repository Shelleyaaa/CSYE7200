...
object WebCrawler extends App {
  ...
  def wget(u: URL): Future[Seq[URL]] = {
    ...
    // 16 points.
    def getURLs(ns: Node): Seq[Try[URL]] = for (n <- ns \\ "a" map (_ \ "@href")) yield createURL(Option(u), n.toString) // TO BE IMPLEMENTED
    ...
    // 9 points.
    for (s <- getURLContent(u); l <- MonadOps.asFuture(getLinks(s))) yield l // TO BE IMPLEMENTED
  }
  ...
  def wget(us: Seq[URL]): Future[Seq[Either[Throwable, Seq[URL]]]] = {
    ...
    // TO BE IMPLEMENTED
    val seq = for (u <- us2) yield MonadOps.sequence(wget(u))
    Future.sequence(seq)
  }
